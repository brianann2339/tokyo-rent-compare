/**
 * 跨欄位與範圍的不變式。違反代表「這個值不能拿去比較或排序」，
 * 但**不代表它是我們編的**——2026-09-06 逐筆核對原始 HTML：14000㎡
 * （tokyosharehouse 3605「広さ: 14000 ㎡」）、662.24㎡、2.5㎡ 都是原站
 * 自己寫在頁面上的數字，我們的解析忠實。
 *
 * 所以處置是「保留原值 ＋ 標記 ＋ 計入健康報告」，不是降為 conflicting：
 * Field 的 known:false 是在斷言「我們不知道這個值」，對一個原站白紙黑字
 * 寫著的數字這句話是假的；而 why:'conflicting' 在 field.ts 的定義是
 * 「多層解析結果不一致」——那是我們解析器的故障訊號，拿來承接「原站自己
 * 打錯」會讓健康監控指錯地方。
 * 落地位置：build-data.ts 的 checkUnit()——違反的房間打上 FLAG.invariantViolation、
 * 違反明細寫進該房的 provenance，前端可據此把它排除在單價排序之外。
 *
 ...
 * 上限 1,000 萬：2026-08 實測港区有 300 萬〜370 萬/月的真實豪宅盤
 * （三田ガーデンヒルズ、白金台、南青山，srcText 原文「賃料 300万円」），
 * 原本的 300 萬上限把真實資料當錯誤丟掉。1,000 萬仍遠高於市場頂端，
 * 足以攔住真正的解析錯誤（電話號碼、坪數誤讀）。
 */
export const RENT_MIN_JPY = 10_000;
export const RENT_MAX_JPY = 10_000_000;
