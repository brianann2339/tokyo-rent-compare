 ...
 * 為什麼是這三区而不是 23 区全部（2026-08-16 實測數字）：
 *   千代田区 20 頁 / 中央区 40 頁 / 港区 59 頁（每頁 pc=50 棟）＝ 共 119 個列表頁。
 *   以 5 秒間隔約 10 分鐘跑完，對站方負載可接受。
 *   23 区全量估計 1,500–2,500 個列表頁（2–3.5 小時、數十萬間房），
 *   遠超過「首版」該有的規模，也會撐爆前端資料預算。
...
function parseBuildingBlock(block: string): SuumoBuilding | null {
  ...
  // 有 <tr> 卻一列都解不出來＝解析器壞了，不是「這棟沒有空房」。大聲失敗。
  const trCount = (block.match(/<tr class="js-cassette_link">/g) ?? []).length;
  if (trCount > 0 && rows.length === 0) {
    throw new Error(`[suumo] 「${name}」有 ${trCount} 個房間列但一列都解析不出來——一覧頁版型可能已改版`);
  }
